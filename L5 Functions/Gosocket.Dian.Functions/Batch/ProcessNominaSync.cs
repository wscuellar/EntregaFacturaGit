...
{
	public static class ProcessNominaSync
	{
		...
		[FunctionName("ProcessNominaSync")]
		...
		// public static async Task Run([QueueTrigger(queueName, Connection = "GlobalStorage")] string myQueueItem, TraceWriter log)
		{
			...
			try
			{
				...
				List<GlobalDocValidatorTracking> ListErr = new List<GlobalDocValidatorTracking>();
				...
				{
					Content = new StringContent(ListErr.Count()==0?"OK documento sin errores": JsonConvert.SerializeObject(ListErr),
